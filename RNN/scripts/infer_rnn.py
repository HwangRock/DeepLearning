...
from RNN.models.nmt_rnn import EncoderLSTM_Att, DecoderLSTM_Att

from tqdm import tqdm

from RNN.utils.text_prepro import text_to_indices, clean_str
from RNN.utils.data_prepro import TranslateDataset, collate
...
def main():
    ...
    if params['task'] == "EN2DE":
        for i in range(params["batch_size"]):
            test_source_text.append(clean_str("Some people are just weird", True))
            test_target_text.append(clean_str("Manche Leute sind einfach komisch", True))

    elif params['task'] == "EN2FR":
        for i in range(params["batch_size"]):
            test_source_text.append(clean_str("Some people are just weird", True))
            test_target_text.append(clean_str("Certaines personnes sont simplement bizarres", True))
    ...
    encoder = EncoderLSTM_Att(vocab_size_source, params['embedding_dim'], params['model_params_rnn']['hidden_size']).to(device)
